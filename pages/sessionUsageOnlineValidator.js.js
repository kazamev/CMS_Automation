...
export class WeeklySessionUsageOnlineValidator {
...
// Calendar date selection (uses your function logic)
  ...
  const startBtn = this.page.locator(
    `//button[.//div[text()='${startDay}']]`
  );
  await startBtn.waitFor({ state: "visible" });
  await startBtn.click();

  // 🔹 If month changed for end date, update calendar
  ...
  // 🔹 Click End Day (Yesterday)
  const endBtn = this.page.locator(
    `//button[.//div[text()='${endDay}']]`
  );
  await endBtn.waitFor({ state: "visible" });
  await endBtn.click();

  console.log("One week selected successfully");
}


//Download  Report with fixed filename
async downloadSessionReport() {
      const downloadPromise = this.page.waitForEvent("download", { timeout: 900000 });
    const downloadBtn = this.page.locator("//button[normalize-space()='Generate Report']");
    await downloadBtn.waitFor({ timeout:60000});
    await downloadBtn.click();
    const download = await downloadPromise;
      this.page.waitForEvent('download', { timeout: 900000 });
...
async verifyDashboardKPIWithChargerExcel(filePath4, sessionKpi, usageKpi) {
    ...
    if (sessionKpi !== excelSessions) {
        console.log(
            `🔴 Sessions mismatch : KPI Session Count: ${sessionKpi},not matched the Session count in ChargerExcel: ${excelSessions}`
        );
    } else {
    console.log(
        `🟢 Sessions matched : KPI Session Count: ${sessionKpi},matched with ChargerExcel Session Count: ${excelSessions}`
    );
}
    ...
    if (USAGEKPI != excelUsageMW) {
        console.log(
            `🔴 Usage mismatch : Dashboard KPI Usage: ${USAGEKPI} MW,does not match the Charger Excel: ${excelUsageMW} MW`
        );
    } else {
    console.log(
        `🟢 Usage matched : Dashboard KPI Usage: ${USAGEKPI} MW, match with Charger Excel Usage: ${excelUsageMW} MW`
    );
}
