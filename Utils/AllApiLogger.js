...
export function attachAllApiLogger(page) {

  const logDir = path.join(process.cwd(), 'all-api-logs');
  if (!fs.existsSync(logDir)) {
    fs.mkdirSync(logDir, { recursive: true });
  }

  const logFile = path.join(logDir, 'all-console.log');
  ...
  const isBusinessApi = url =>
    (url.includes('/api/v1/') || url.includes('/api/v2/') || url.includes('/api/v3/') )&&
 ...
 printAllApis(testName = '') {
  ...
  if (index === rows.length - 1) {
    table += line + '\n';   // only add line once at bottom
  }
});


  const title = `==== ALL BUSINESS API CALLS : ${testName} ====\n`;
